...
async function createAudioGraph() {
  ...
  await context.audioWorklet.addModule(new URL(
    "./src/hambone-processor.js?v=hambone-tract-20260829-1",
    import.meta.url,
  ));
...
function queueSoundVisual(soundId, velocity, delaySeconds = 0, step = null) {
  visualQueue.push({
    type: "sound",
...
function postStrike(soundId, velocity = 1, delaySeconds = 0, step = null) {
  ...
  queueSoundVisual(soundId, velocity, boundedDelay, step);
  return true;
}

async function triggerSound(soundId, velocity = 1) {
  const sound = hamboneSound(soundId);
  if (!(await ensureAudio())) return false;
  postStrike(sound.id, velocity);
  ...
  while (nextStepTime < audioContext.currentTime + lookaheadSeconds) {
    ...
    if (event) {
      ...
      const velocityMotion = 1 + deterministicHumanize(absoluteStep, soundIndex + 23)
        * state.humanize * 0.22;
      postStrike(event.soundId, event.velocity * velocityMotion, delaySeconds, step);
    }
    visualQueue.push({ type: "step", step, due: performance.now() + delaySeconds * 1000 });
...
function scatterPattern() {
  ...
  markPatternCustom();
  renderPattern();
  announce("A new one-mouth pattern was scattered across the grid");
}
...
function buildSequenceGrid() {
  ...
  HAMBONE_SOUNDS.forEach((sound, rowIndex) => {
    ...
    trigger.setAttribute("aria-label", `Move the one mouth to ${sound.label}: ${sound.subtitle}`);
...
function physicalTelemetryPose(articulation) {
  const pose = { ...state };
...
function flushVisualQueue(now) {
  ...
  for (const event of visualQueue) {
    ...
    const durations = { bop: 210, boop: 300, pop: 190, tlik: 150, shh: 250, shack: 340, slap: 270, pff: 300 };
    soundAnimation = {
      soundId: sound.id,
      velocity: event.velocity,
      start: now,
      duration: prefersReducedMotion ? 90 : durations[sound.id],
    };
...
function drawAirPlume(context, layout, motion, now) {
  const amount = Math.max(motion.shh, motion.shack * 0.62, motion.pff * 0.44);
...
function drawFace(context, layout, pose, motion, now) {
  ...
  const slap = motion.slap;
  const pop = motion.pop;
  const shack = motion.shack;
  const wobble = (slap * -1 + pop * 0.38 + shack * 0.18) * (0.018 + state.silliness * 0.025);
  context.save();
  ...
  for (const side of [-1, 1]) {
    ...
    const gazeX = Math.sin(gazePhase + side * 2.35) * eyeRx * clownEnergy * 0.31
      - side * eyeRx * 0.12
      + wobble * 55;
  ...
  const mouthPulse = Math.max(motion.bop * 0.48, motion.boop * 0.68, motion.shack, motion.pff * 0.5);
  const roundedGesture = motion.boop * 0.9 + motion.pop * 0.46 + motion.pff * 0.38;
  const spreadGesture = motion.shh * 0.48 + motion.tlik * 0.22 + motion.shack * 0.16;
  const flutter = motion.pff * Math.sin(now * 0.045) * (0.08 + state.silliness * 0.06);
  ...
  let liveOpening = opening * clamp(
    1 + mouthPulse * (0.75 + state.silliness * 0.45) + motion.tlik * 0.42 - motion.shh * 0.18 + flutter,
    0.12,
    3.2,
  );
  ...
  const upperLipReach = liveOpening + lipThickness * (1.05 + motion.bop * 0.2);
  const lowerLipReach = liveOpening + lipThickness * (1.42 + motion.pff * 0.25);
...
function buildHitGeometry(layout, pose) {
  ...
  for (const point of [...hotspots, ...handles]) {
...
function drawHandles(context) {
  for (const handle of handles) {
    ...
    context.beginPath();
    context.arc(handle.x, handle.y, handle.r + (selected ? 2 : 0), 0, Math.PI * 2);
    context.fill();
    context.stroke();
...
function drawStage(now = performance.now()) {
  ...
  const targetPose = physicalStatus
    ? physicalTelemetryPose(physicalStatus)
    : isSpeaking
      ? hambonePoseForSound(strongest[0], state, Math.min(0.82, strongest[1] * 0.72))
      : state;
...
function bindControls() {
  ...
  globalThis.addEventListener("keydown", (event) => {
    ...
    const sound = HAMBONE_SOUNDS.find(({ key }) => key === event.key);
    if (!sound) return;
    event.preventDefault();
