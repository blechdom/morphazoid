...
function isWaxMidiOnly() {
  return document.documentElement?.dataset?.morphazoidWaxOutputMode === "midi";
}

function enableAudio() {
  if (isWaxMidiOnly()) return Promise.resolve(false);
...
async function triggerVoice(voice) {
  if (isWaxMidiOnly()) return;
  const lifecycleGeneration = audioLifecycleGeneration;
  if ((!state.audioOn || !audio.context) && !await enableAudio()) return;
  if (lifecycleGeneration !== audioLifecycleGeneration || isWaxMidiOnly()) return;
  try {
...
function registerMidiClient() {
  ...
  unregisterMidi = midiManager.registerClient({
    ...
    onPrepareEnable: () => {
      if (!state.audioOn && !isWaxMidiOnly()) void enableAudio();
    },
