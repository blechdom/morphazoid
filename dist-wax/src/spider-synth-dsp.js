import { SPIDER_BODY_GROUPS, getSpiderJointBodyGroup, getSpiderBodyGroupId } from './spider-synth-body.js?v=456b92d1a726';
import { SPIDER_JOINTS, SPIDER_MOTION_PRESETS, normalizeSpiderMotion, createSpiderWeb, createSpiderFrame, writeSpiderPose, writeSpiderFrame, spiderStringFrequency } from './spider-synth-model.js?v=456b92d1a726';
import { SpiderMidiPerformance } from './spider-synth-midi.js?v=456b92d1a726';
import { SpiderStrings, SpiderOutputGuard } from './spider-synth-string.js?v=456b92d1a726';
export { SPIDER_BODY_GROUPS, getSpiderBodyGroupId } from './spider-synth-body.js?v=456b92d1a726';
...
export const SPIDER_SOUND_DEFAULTS = Object.freeze({ level: .5, tension: 1, damping: .35, coupling: .12, brightness: .65, decay: 1.8, tune: 1, body: .6, voice: .65, pan: 0 });
const RANGES = { level: [0,.8], tension: [.25,4], damping: [0,1], coupling: [0,.4], brightness: [0,1], decay: [.08,6], tune: [.5,2], body: [0,1], voice: [0,1], pan: [-1,1] };
...
const DEFAULT_SOURCES = ['click','hollow','sub','thumb','clack','shimmer','silk','glass'];
...
function patch(id,label,sources,sound={}) { const bodyMix=createDefaultSpiderBodyMix().map((row,i)=>({...row,source:sources[i]??row.source}));return Object.freeze({id,label,sound:Object.freeze(normalizeSpiderSound(sound)),bodyMix:Object.freeze(bodyMix.map(Object.freeze))}); }
export const SPIDER_SOUND_PRESETS=Object.freeze([
 patch('orb-silk','Orb silk',DEFAULT_SOURCES),
...
export const SPIDER_MOTION_SOUND_PRESETS=Object.freeze(SPIDER_MOTION_PRESETS.map((motion,i)=>{
 ...
 return Object.freeze({id:motion.id,label:motion.label,sound:Object.freeze(normalizeSpiderSound({...source.sound,tension:source.sound.tension*(.9+(i%4)*.09),tune:source.sound.tune*(.94+(i%3)*.05)})),bodyMix:Object.freeze(bodyMix.map(Object.freeze))});
}));
export function getSpiderMotionSound(id) { const found=SPIDER_MOTION_SOUND_PRESETS.find(p=>p.id===id)??SPIDER_MOTION_SOUND_PRESETS[0];return {id:found.id,label:found.label,sound:{...found.sound},bodyMix:found.bodyMix.map(r=>({...r}))}; }
export function createRandomSpiderSound(seed=1) { let n=(finite(seed,1)|0)||1;const random=()=>{n^=n<<13;n^=n>>>17;n^=n<<5;return(n>>>0)/4294967296;};return {sound:normalizeSpiderSound({tension:.4+random()*2,decay:.3+random()*3,damping:.15+random()*.65,coupling:random()*.3,brightness:.25+random()*.7,tune:.6+random()*.9,body:.25+random()*.65,voice:.5+random()*.25,pan:(random()-.5)*.5}),bodyMix:createDefaultSpiderBodyMix().map(r=>({...r,source:SPIDER_BODY_SOURCES[Math.floor(random()*16)].id,level:.2+random()*.5}))}; }
...
class BodyVoice {
 constructor(rate,index) { this.rate=rate;this.index=index;this.source=0;this.oldSource=0;this.fade=1;this.level=0;this.targetLevel=0;this.phase=0;this.modPhase=.13;this.subPhase=.7;this.env=0;this.age=0;this.manual=0;this.frequency=140;this.targetFrequency=140;this.gate=0;this.targetGate=0;this.x=0;this.y=0;this.z=0;this.low=0;this.filtered=0;this.eventFrequency=0;this.decayCoefficient=.99;this.filterCoefficient=.25;this.panL=.7;this.panR=.7;this.seed=(0x4714153^index*7393)|0;this.output=new Float64Array(16);this.smooth=1-Math.exp(-1/(rate*.022)); }
 ...
 sample(still,sound){
  this.level+=(this.targetLevel-this.level)*this.smooth;this.gate+=(this.targetGate-this.gate)*this.smooth;this.frequency+=(this.targetFrequency-this.frequency)*this.smooth;
...
export class SpiderSynthDsp {
 constructor(sampleRate=48000){
  ...
  this.sound=normalizeSpiderSound();this.smooth=normalizeSpiderSound();this.motion=normalizeSpiderMotion();this.webSettings={spokes:16,rings:10,tension:1,seed:1};this.web=createSpiderWeb(this.webSettings);this.setNeighbors();
  ...
  this.levels=new Float64Array(8);this.strings=new SpiderStrings(this.sampleRate);this.guard=new SpiderOutputGuard();
  ...
  this.groupPose=new Float64Array(24);this.groupDistance=new Float64Array(8);this.distances=new Float64Array(8);this.groupCounts=new Uint8Array(8);this.jointGroups=new Uint8Array(SPIDER_JOINTS.length);for(let i=0;i<SPIDER_JOINTS.length;i++){const group=getSpiderJointBodyGroup(SPIDER_JOINTS[i]);this.jointGroups[i]=group;this.groupCounts[group]++;}
  ...
  this.interactionJoint=-1;this.interactionActive=false;this.midiGates=new Float64Array(8);this.contactEvents=0;this.pluckEvents=0;this.lastContactTime=-1;this.eventCursor=0;this.recentEvents=Array.from({length:16},()=>({id:0,segmentId:0,u:.5,velocity:0,audioTime:-1,source:'contact'}));
 ...
 update(value={}){
  ...
  if(value.webSettings){
   const settings={...this.webSettings,...value.webSettings};settings.spokes=Math.round(clamp(settings.spokes,8,24));settings.rings=Math.round(clamp(settings.rings,3,16));settings.seed=finite(settings.seed,1)|0;
   ...
   const changed=settings.spokes!==this.webSettings.spokes||settings.rings!==this.webSettings.rings||settings.seed!==this.webSettings.seed;
   this.webSettings=settings;if(changed){this.web=createSpiderWeb({...settings,tension:this.sound.tension});this.setNeighbors();this.strings.release();this.primed=false;}
  }
 ...
 emit(segmentId,u,velocity,source){const event=this.recentEvents[this.eventCursor];event.id=++this.pluckEvents;event.segmentId=segmentId;event.u=u;event.velocity=velocity;event.audioTime=this.audioTime;event.source=source;this.eventCursor=(this.eventCursor+1)%this.recentEvents.length;}
 pluck({segmentId,u=.5,velocity=.65,angle=Math.PI/2,source='gesture'}={}){
  ...
  const accepted=this.pluckString(id,clamp(u,0,1),strength,finite(angle),-1,source);
 ...
 pluckString(id,u,strength,angle,owner=-1,source='contact',midiRatio=1,couple=true,midiOwner=null){
  ...
  let accepted=false;
  if(voice.source<5)accepted=this.strings.pluck(frequency,amount,clamp(x*.8+this.smooth.pan+(owner>=0?voice.z*.3:0),-1,1),this.smooth,group,id,voice.source,u,midiOwner,expression);
  else{voice.eventFrequency=frequency/this.smooth.tune;voice.targetFrequency=frequency;voice.excite(amount);accepted=true;}
  if(accepted){this.emit(id,u,strength,source);if(couple&&this.smooth.coupling>0){const neighbor=this.neighbors[id];this.pluckString(neighbor,.5,strength*this.smooth.coupling,angle,owner,'coupling',midiRatio,false,midiOwner);}}
  return accepted;
 }
 ...
 control(){
  ...
  writeSpiderFrame(this.time,this.motion,this.web,this.frame,this.pose,this.midiPerformance.poseOffsets);this.groupPose.fill(0);this.groupDistance.fill(0);
  ...
  for(let i=0;i<8;i++){
   ...
   if(this.enabled&&this.distances[i]>.12&&!(this.playing&&i===0)){this.distances[i]%=.12;this.exciteGroup(i,Math.min(.85,.25+this.groupDistance[i]*2),'gesture');}
   const foot=this.frame.feet[i];if(this.primed&&this.playing&&this.enabled&&foot.stance&&foot.step>this.steps[i]){this.contactEvents++;this.lastContactTime=this.time;this.exciteGroup(0,foot.impact,'contact',i);this.pluckString(foot.segmentId,foot.u,foot.impact,foot.angle,-1,'contact');}
   const k=i*3,dx=foot.x-this.previousFeet[k],dy=foot.y-this.previousFeet[k+1],dz=foot.z-this.previousFeet[k+2];
   if(this.primed&&this.overlayPrimed&&!this.playing&&this.enabled){this.footDistances[i]+=Math.hypot(dx,dy,dz);if(this.footDistances[i]>.012){this.footDistances[i]%=.012;this.pluckString(foot.segmentId,foot.u,Math.min(.7,.25+Math.hypot(dx,dz)*15),Math.atan2(dz,dx),-1,'gesture');}}
 ...
 render(left,right,audioTime){
  ...
  for(let i=0;i<count;i++){
   ...
   for(const p of this.prey)if(p.active&&this.audioTime>=p.time){p.active=false;if(this.enabled&&this.audioTime-p.time<.1)this.pluckString(p.segmentId,p.u,p.velocity,p.angle,-1,'prey');}
   ...
   this.strings.sample(this.levels,this.smooth.brightness);
   ...
   const rawL=this.strings.left*6+bodyL*2.1+voice*Math.cos((pan+1)*Math.PI/4)+click;
   const rawR=this.strings.right*6+bodyR*2.1+voice*Math.sin((pan+1)*Math.PI/4)+click;
