import { SPIDER_BODY_GROUPS, getSpiderJointBodyGroup } from './spider-synth-body.js?v=456b92d1a726';
import { constrainSpiderPose } from './spider-synth-model.js?v=456b92d1a726';

const AXES = ['x', 'y', 'z'];
const CONTROL_AMOUNTS = Object.freeze([.42, .24, .3, .5, .35, .12, 0, 0]);
const MAX_SCOPES = 64;
export const SPIDER_MIDI_MAX_VOICES = 24;
export const SPIDER_MIDI_ROOT_NOTE = 60;
export const SPIDER_MIDI_TARGETS = Object.freeze([
  ['leg_left_1', 0, -1], ['leg_left_2', 0, -1], ['leg_left_3', 0, -1], ['leg_left_4', 0, -1],
  ['leg_right_1', 0, 1], ['leg_right_2', 0, 1], ['leg_right_3', 0, 1], ['leg_right_4', 0, 1],
  ['cephalothorax', 1, 0], ['abdomen', 2, 0], ['pedipalps', 3, 0], ['chelicerae', 4, 0],
].map(([id, group, side]) => Object.freeze({ id, group, groupId: SPIDER_BODY_GROUPS[group].id, side })));
export const SPIDER_MIDI_GESTURES = Object.freeze(SPIDER_MIDI_TARGETS.map((target, index) => Object.freeze({ ...target,
  label: ['Left front leg', 'Left second leg', 'Left third leg', 'Left rear leg',
    'Right front leg', 'Right second leg', 'Right third leg', 'Right rear leg',
    'Head / thorax', 'Abdomen', 'Palps', 'Fangs'][index],
})));
...
const groupIndex = value => typeof value === 'number' && Number.isInteger(value) && value >= 0 && value < 8
  ? value : SPIDER_BODY_GROUPS.findIndex(group => group.id === value);
...
export function normalizeSpiderMidiMessage(input) {
 ...
 */
export class SpiderMidiPerformance {
  constructor({ attack = .012, release = .16, maxVoices = SPIDER_MIDI_MAX_VOICES } = {}) {
    this.attack = clamp(attack, .004, .2, .012); this.release = clamp(release, .025, 1, .16);
    this.maxVoices = Math.round(clamp(maxVoices, 1, SPIDER_MIDI_MAX_VOICES, SPIDER_MIDI_MAX_VOICES));
    ...
    this.poseOffsets = new Float64Array(0);
  ...
  handle(input, now = this.now) {
    const message = normalizeSpiderMidiMessage(input);
    ...
    if (message.type === 'noteOn') {
      ...
      Object.assign(voice, { active: true, sourceId: message.sourceId, channel: message.channel, note: message.note,
        ...
        target: message.note % 12, group: SPIDER_MIDI_TARGETS[message.note % 12].group, velocity: message.velocity / 127,
  ...
  sample(now = this.now, tempo = 120) {
    ...
    for (let group = 0; group < 8; group += 1) {
      ...
      out.pressures[group] = Math.max(scope.pressure, voice.pressure); out.pans[group] = SPIDER_MIDI_TARGETS[voice.target].side * .6;
      out.frequencies[group] = 440 * 2 ** ((voice.note - 69 + scope.bend * 2) / 12);
      out.pitchRatios[group] = 2 ** ((voice.note - SPIDER_MIDI_ROOT_NOTE + scope.bend * 2) / 12);
  ...
  setJoints(joints) {
    ...
    this.poseOffsets = new Float64Array(count * 3);
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const group = getSpiderJointBodyGroup(joint); const side = /left/.test(name) ? -1 : /right/.test(name) ? 1 : 0;
      this.jointGroups[i] = group; this.jointSides[i] = side;
      const leg = Number(name.match(/leg_(?:left|right)_([1-4])/)?.[1] ?? 1) - 1;
      const target = group === 0 ? (side < 0 ? leg : side > 0 ? 4 + leg : -1)
        : group >= 1 && group <= 4 ? group + 7 : -1;
      this.jointTargets[i] = target;
      this.jointPhases[i] = leg * .43;
      this.jointScales[i] = /hip/.test(name) ? .65 : /tip|ankle/.test(name) ? .45 : 1;
    ...
    constrainSpiderPose(neutral, joints, neutral);
    return this;
  }
  applyPose(pose, joints, now = this.now, tempo = 120, intensity = 1) {
    ...
    for (let i = 0; i < joints.length; i += 1) {
      ...
      const note = voiceIndex >= 0 ? this.voices[voiceIndex].note : SPIDER_MIDI_ROOT_NOTE;
      ...
      if (group === 0) { x = sine * .24; y = Math.sin(phase + .65) * .28 * sign; z = cosine * .18 * sign; }
      else if (group === 1) { x = sine * .15; y = cosine * .18; z = Math.sin(phase * .5) * .12; }
      else if (group === 2) { x = sine * .24; y = cosine * .16; z = sine * .12; }
      else if (group === 3) { x = sine * .36; y = cosine * .28 * sign; z = sine * .22 * sign; }
      else if (group === 4) { x = sine * .3; y = cosine * .16 * sign; z = sine * .18 * sign; }
      const scale = amount * this.jointScales[i];
      const controlAmount = CONTROL_AMOUNTS[group] * strength;
      // Expose the additive request separately from the clamped rendered pose.
      // Shared contacts evaluate it at touchdown time to keep stance planted.
      this.poseOffsets[k] = x * scale + this.controlValues[c] * controlAmount;
      this.poseOffsets[k + 1] = y * scale + this.controlValues[c + 1] * controlAmount;
      this.poseOffsets[k + 2] = z * scale + this.controlValues[c + 2] * controlAmount;
      pose[k] += this.poseOffsets[k];
      pose[k + 1] += this.poseOffsets[k + 1];
      pose[k + 2] += this.poseOffsets[k + 2];

    }
    return constrainSpiderPose(pose, joints, pose);
  }
  ...
  restore(snapshot, { timeOffset = 0 } = {}) {
    ...
    for (let i = 0; i < this.voices.length; i += 1) {
      ...
      Object.assign(this.voices[i], { active: value.active === true && this.scopes[scope].active, sourceId: source(value.sourceId), channel: Math.round(clamp(value.channel, 0, 15)),
        ...
        group: SPIDER_MIDI_TARGETS[note % 12].group, velocity: clamp(value.velocity, 0, 1), pressure: clamp(value.pressure, 0, 1),
