...
import { ROACH_BODY_GROUPS } from './roach-synth-body.js?v=1fe61cc28159';
import { ROACH_MIDI_GESTURES } from './roach-synth-midi.js?v=1fe61cc28159';

export const ROACH_MIDI_ROUTE_DEFAULTS = Object.freeze([
  'param:intensity', 'param:brightness', 'param:resonance', 'param:crunch',
  'joint:covers:y', 'joint:hindwings:y', 'joint:head:x', 'joint:antennae:y',
]);
const KEY = 'morphazoid:roach-midi:v1';
const PARAMS = [
  ['intensity', 'Movement'], ['tempo', 'Tempo'], ['pitch', 'Pitch'], ['brightness', 'Filter'],
  ['resonance', 'Resonance'], ['crunch', 'Crust'], ['vowel', 'Vowel'], ['wingRate', 'Wing rate'],
  ['rhythm', 'Rattle rate'], ['pan', 'Pan'], ['voice', 'Voice level'], ['level', 'Master level'],
];
const ACTIONS = [
...
/** Own only Roach's public MIDI event; the shared toolbar owns permission,
 * controller profiles and keyboard input in both browser and WAX builds. */
export function createRoachMidiControls({ audio, setPlaying, selectGroup, onVisual,
  runtime = window, document: doc = runtime.document }) {
  const el = id => doc.getElementById(id);
  const listeners = new AbortController(), options = { signal: listeners.signal };
  let disposed = false, routes = [...ROACH_MIDI_ROUTE_DEFAULTS];
  ...
  const groups = [
    ...
    ['Body rotations', ROACH_BODY_GROUPS.flatMap(({ id, label }) => ['x', 'y', 'z'].map(axis => [`joint:${id}:${axis}`, `${label} · ${axis.toUpperCase()}`]))],
    ['Body levels', ROACH_BODY_GROUPS.map(({ id, label }) => [`mix:${id}`, `${label} level`])],
    ['Body sounds', ROACH_BODY_GROUPS.map(({ id, label }) => [`source:${id}`, `${label} sound`])],
  ...
  function range(id, normalized) {
    ...
    const raw = id === 'pitch' || id === 'wingRate' ? low * (high / low) ** normalized : low + (high - low) * normalized;
  ...
  function handle(event) {
    ...
    if (!message || (detail.routeId && detail.routeId !== 'roach-synth') || disposed) return;
    ...
    if (type === 'noteOn' || type === 'noteOff') {
      ...
      if (attack) {
        const gesture = ROACH_MIDI_GESTURES[message.note % 12];
        lastLabel = `${NOTE_NAMES[message.note % 12]}${Math.floor(message.note / 12) - 1} · ${gesture.label}`;
        el('midiStatus').textContent = lastLabel;
        selectGroup(gesture.groupId, gesture.side);
      }
      wake(); return;
    }
    if (type === 'controlChange') {
      ...
      const parameter = { 1: 'intensity', 7: 'level', 10: 'pan', 71: 'resonance', 74: 'brightness' }[cc];
  ...
  for (const [i, gesture] of ROACH_MIDI_GESTURES.entries()) {
  ...
  el('midiResetRoutes').addEventListener('click', () => {
    panic(); routes = [...ROACH_MIDI_ROUTE_DEFAULTS]; routes.forEach((id, i) => { el(`midiRoute${i}`).value = id; }); save();
  }, options);
