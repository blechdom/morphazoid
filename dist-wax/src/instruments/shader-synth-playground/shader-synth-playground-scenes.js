...
export function createShaderPlaygroundScenes(moduleSpecs = []) {
  ...
  const scenes = [
    ...
    scene(221, "mirror-fold-constellation", {
      name: "Mirror Fold Constellation",
      description: "A nested mirror traversal fans one pitch and gate path into a soft waveform and an inharmonic PM bell, then spreads their shared geometry through a slow chorus and long room.",
      family: "folded-domain-ensemble",
      character: "mirrored major constellations",
      category: "geometry",
      route: "Mirror Fold → Oscillator VCA + PM VCA → Mix → Chorus → Reverb → Output",
    }, [
      node("geometry", "mirror-fold-sequencer", 20, 190, { rate: 4.05, length: 31, folds: 5, symmetry: 7, warp: 0.317, scale: 1, octaves: 3, width: 0.46 }),
      node("wave", "oscillator", 245, 35, { frequency: 55, waveform: 1, shape: 0.5, level: 0.48, pmDepth: 1.8 }),
      node("wave-vca", "vca", 485, 35, { base: 0, depth: 1, drive: 1.18 }),
      node("bell", "fm", 245, 255, { frequency: 55, ratio: 2.381, index: 2.9, level: 0.4, modDepth: 3.6 }),
      node("bell-vca", "vca", 485, 255, { base: 0, depth: 1, drive: 1.26 }),
      node("blend", "mix", 720, 145, { balance: 0.08, level: 0.82 }),
      node("chorale", "chorus", 945, 145, { base: 22, depth: 7.2, rate: 0.18, mix: 0.54, voices: 6, spread: 0.96, drift: 0.48, level: 0.86 }),
      node("room", "reverb", 1175, 145, { size: 7.4, decay: 10.2, taps: 64, mix: 0.58, predelay: 0.075, width: 1, tone: 0.54, level: 0.8 }),
      output(1405, 145, 0.54),
    ], [
      edge("geometry", "pitch", "wave", "pitch"), edge("geometry", "gate", "wave-vca", "cv"), edge("wave", "out", "wave-vca", "signal"),
      edge("geometry", "pitch", "bell", "pitch"), edge("geometry", "gate", "bell", "index"), edge("geometry", "gate", "bell-vca", "cv"), edge("bell", "out", "bell-vca", "signal"),
      edge("wave-vca", "out", "blend", "a"), edge("bell-vca", "out", "blend", "b"),
      edge("blend", "out", "chorale", "signal"), edge("chorale", "out", "room", "signal"), edge("room", "out", "out", "signal"),
    ]),

    scene(222, "sdf-boundary-forge", {
      name: "SDF Boundary Forge",
      description: "A rotating ring boundary excites a modal plate and opens a particle stream; multiplying the two gated branches turns each contour crossing into a different metallic spray.",
      family: "distance-field-percussion",
      character: "ring-crossing metal weather",
      category: "geometry",
      route: "SDF Shape → Modal VCA × Particle VCA → Ring Modulator → Fold → Delay → Output",
    }, [
      node("geometry", "sdf-orbit-sequencer", 20, 185, { rate: 2.85, shape: 4, size: 0.64, band: 0.038, rotation: 0.23, scale: 5, octaves: 3, seed: 39019 }),
      node("metal", "modal-metal", 245, 35, { frequency: 72, rate: 2.85, decay: 1.18, inharmonicity: 0.88, modes: 28, brightness: 1.34, strikeDepth: 1, level: 0.48 }),
      node("metal-vca", "vca", 485, 35, { base: 0, depth: 1, drive: 1.42 }),
      node("cloud", "particle-cloud", 245, 260, { frequency: 96, rate: 42, size: 0.026, density: 0.62, spread: 17, stereo: 0.94, seed: 18121, level: 0.42 }),
      node("cloud-vca", "vca", 485, 260, { base: 0, depth: 1, drive: 1.3 }),
      node("multiply", "ring", 715, 145, { amount: 0.7, level: 0.9 }),
      node("fold", "fold", 925, 145, { drive: 4.8, fold: 0.76, mix: 0.68, symmetry: -0.08 }),
      node("echo", "delay", 1140, 145, { time: 0.173, repeats: 6, decay: 0.7, mix: 0.48, spread: 1, tone: 0.26, pattern: 0.68, level: 0.84 }),
      output(1370, 145, 0.56),
    ], [
      edge("geometry", "pitch", "metal", "pitch"), edge("geometry", "gate", "metal", "strike"), edge("geometry", "gate", "metal-vca", "cv"), edge("metal", "out", "metal-vca", "signal"),
      edge("geometry", "pitch", "cloud", "pitch"), edge("geometry", "gate", "cloud-vca", "cv"), edge("cloud", "out", "cloud-vca", "signal"),
      edge("metal-vca", "out", "multiply", "a"), edge("cloud-vca", "out", "multiply", "b"),
      edge("multiply", "out", "fold", "signal"), edge("fold", "out", "echo", "signal"), edge("echo", "out", "out", "signal"),
    ]),

    scene(223, "kaleidoscope-star-choir", {
      name: "Kaleidoscope Star Choir",
      description: "A quarter-tone star polygon articulates three independently colored voices, preserving the mirrored angular order while a shifted spectral reconstruction stretches it into a radiant choir.",
      family: "polar-star-ensemble",
      character: "microtonal mirrored choir",
      category: "geometry",
      route: "Polar Kaleidoscope → Supersaw VCA + Formant VCA + FOF VCA → Three-way Sum → Spectral Resynth → Reverb → Output",
    }, [
      node("geometry", "polar-kaleidoscope-sequencer", 20, 210, { rate: 6.25, length: 28, sectors: 8, stride: 11, rotation: 0.187, scale: 6, octaves: 4, width: 0.61 }),
      node("saw", "supersaw", 245, 20, { frequency: 41.2, voices: 5, detune: 12, spread: 0.76, driftRate: 0.12, driftDepth: 0.17, waveform: 1, level: 0.38 }),
      node("saw-vca", "vca", 475, 20, { base: 0, depth: 1, drive: 1.16 }),
      node("vowel", "formant-bank", 245, 195, { frequency: 55, vowel: 0.56, harmonics: 28, bandwidth: 205, tilt: 0.76, breath: 0.1, cvDepth: 0.72, level: 0.42 }),
      node("vowel-vca", "vca", 475, 195, { base: 0, depth: 1, drive: 1.24 }),
      node("fof", "fof-voice", 245, 370, { frequency: 36.71, formant: 1260, bandwidth: 150, duration: 0.018, second: 2.62, breath: 0.16, stereo: 0.58, level: 0.4 }),
      node("fof-vca", "vca", 475, 370, { base: 0, depth: 1, drive: 1.2 }),
      node("choir", "sum-3", 710, 195, { a: 0.78, b: 1.08, c: 0.88, level: 0.78 }),
      node("spectrum", "spectral-resynth", 930, 195, { window: 128, bins: 24, shift: 1.28, mix: 0.58, smear: 0.72, tilt: 0.82, width: 1, level: 0.78 }),
      node("room", "reverb", 1155, 195, { size: 5.8, decay: 8.6, taps: 60, mix: 0.54, predelay: 0.092, width: 1, tone: 0.62, level: 0.8 }),
      output(1385, 195, 0.5),
    ], [
      edge("geometry", "pitch", "saw", "pitch"), edge("geometry", "gate", "saw-vca", "cv"), edge("saw", "out", "saw-vca", "signal"),
      edge("geometry", "pitch", "vowel", "pitch"), edge("geometry", "gate", "vowel-vca", "cv"), edge("vowel", "out", "vowel-vca", "signal"),
      edge("geometry", "pitch", "fof", "pitch"), edge("geometry", "gate", "fof-vca", "cv"), edge("fof", "out", "fof-vca", "signal"),
      edge("saw-vca", "out", "choir", "a"), edge("vowel-vca", "out", "choir", "b"), edge("fof-vca", "out", "choir", "c"),
      edge("choir", "out", "spectrum", "signal"), edge("spectrum", "out", "room", "signal"), edge("room", "out", "out", "signal"),
    ]),

    scene(224, "voronoi-border-radio", {
      name: "Voronoi Border Radio",
      description: "Jittered cell borders gate a thin waveform, a talking harmonic bank, and a noise carrier; a spectral mask and fragment memory turn their irregular spacing into broken transmissions.",
      family: "voronoi-border-events",
      character: "irregular cellular radio",
      category: "geometry",
      route: "Voronoi Borders → Oscillator VCA + Robot VCA + Noise VCA → Three-way Sum → Spectral Gate → Recombobulator → Output",
    }, [
      node("geometry", "voronoi-cell-sequencer", 20, 205, { rate: 4.45, jitter: 0.94, feature: 1, width: 0.032, seed: 51283, scale: 3, octaves: 3, randomness: 0.92 }),
      node("carrier", "oscillator", 245, 20, { frequency: 61.74, waveform: 3, shape: 0.31, level: 0.42, pmDepth: 1.8 }),
      node("carrier-vca", "vca", 475, 20, { base: 0, depth: 1, drive: 1.3 }),
      node("robot", "robot-voice", 245, 190, { frequency: 46.25, vowel: 0.42, harmonics: 20, bandwidth: 180, robotRate: 11.5, metal: 2.2, cvDepth: 0.65, level: 0.4 }),
      node("robot-vca", "vca", 475, 190, { base: 0, depth: 1, drive: 1.35 }),
      node("static", "noise", 245, 360, { rate: 14500, smooth: 0.05, seed: 60101, level: 0.23 }),
      node("static-vca", "vca", 475, 360, { base: 0, depth: 1, drive: 1.18 }),
      node("broadcast", "sum-3", 710, 190, { a: 0.88, b: 1.02, c: 0.46, level: 0.8 }),
      node("mask", "spectral-gate", 925, 190, { window: 96, bins: 18, threshold: 0.042, mix: 0.78, knee: 0.38, reduction: 0.94, tilt: 1.3, level: 0.88 }),
      node("memory", "recombobulator", 1150, 190, { memory: 2.8, heads: 10, rate: 0.72, mix: 0.62, scatter: 1, fold: 0.38, width: 1, level: 0.8 }),
      output(1380, 190, 0.52),
    ], [
      edge("geometry", "pitch", "carrier", "pitch"), edge("geometry", "gate", "carrier-vca", "cv"), edge("carrier", "out", "carrier-vca", "signal"),
      edge("geometry", "pitch", "robot", "pitch"), edge("geometry", "gate", "robot", "vowel"), edge("geometry", "gate", "robot-vca", "cv"), edge("robot", "out", "robot-vca", "signal"),
      edge("geometry", "gate", "static-vca", "cv"), edge("static", "out", "static-vca", "signal"),
      edge("carrier-vca", "out", "broadcast", "a"), edge("robot-vca", "out", "broadcast", "b"), edge("static-vca", "out", "broadcast", "c"),
      edge("broadcast", "out", "mask", "signal"), edge("mask", "out", "memory", "signal"), edge("memory", "out", "out", "signal"),
    ]),

    scene(225, "truchet-ribbon-duet", {
      name: "Truchet Ribbon Duet",
      description: "A wandering scan across connected arc tiles alternately reveals a stiff plucked string and an acid partial bank, while an independent slow curve moves their balance through flanged echoes.",
      family: "truchet-path-duet",
      character: "curved call-and-response ribbons",
      category: "geometry",
      route: "Truchet Path → Pluck VCA + Spectral Acid VCA → Morph → Flanger → Delay → Output",
    }, [
      node("geometry", "truchet-path-sequencer", 20, 190, { rate: 3.55, length: 61, lane: -0.08, wander: 0.88, width: 0.039, seed: 47057, scale: 4, octaves: 3 }),
      node("pluck", "analytic-plucked-string", 245, 30, { rate: 3.55, frequency: 48.99, modes: 24, pluck: 0.19, decay: 2.2, stiffness: 0.34, stereo: 0.72, level: 0.5 }),
      node("pluck-vca", "vca", 475, 30, { base: 0, depth: 1, drive: 1.18 }),
      node("acid", "spectral-acid", 245, 275, { frequency: 73.42, partials: 38, cutoff: 0.32, resonance: 0.84, tilt: 0.78, drive: 2.9, level: 0.38, cvDepth: 0.72 }),
      node("acid-vca", "vca", 475, 275, { base: 0, depth: 1, drive: 1.3 }),
      node("motion", "lfo", 475, 420, { rate: 0.083, shape: 1, depth: 0.88, offset: 0 }),
      node("ribbon", "morph-crossfade", 710, 145, { morph: 0.5, cvDepth: 0.48, curve: 1.45, gainA: 1, gainB: 0.92, centerTrim: 0.64, level: 0.88 }),
      node("sweep", "flanger", 935, 145, { base: 3.4, depth: 6.8, rate: 0.14, mix: 0.58, stereo: 0.9, polarity: -0.48, tone: 0.12, level: 0.86 }),
      node("echo", "delay", 1160, 145, { time: 0.411, repeats: 5, decay: 0.66, mix: 0.48, spread: 1, tone: 0.3, pattern: 0.44, level: 0.84 }),
      output(1390, 145, 0.54),
    ], [
      edge("geometry", "pitch", "pluck", "pitch"), edge("geometry", "gate", "pluck-vca", "cv"), edge("pluck", "out", "pluck-vca", "signal"),
      edge("geometry", "pitch", "acid", "pitch"), edge("geometry", "gate", "acid-vca", "cv"), edge("acid", "out", "acid-vca", "signal"),
      edge("pluck-vca", "out", "ribbon", "a"), edge("acid-vca", "out", "ribbon", "b"), edge("motion", "out", "ribbon", "morph"),
      edge("ribbon", "out", "sweep", "signal"), edge("sweep", "out", "echo", "signal"), edge("echo", "out", "out", "signal"),
    ]),

    scene(226, "kifs-fractal-foundry", {
      name: "KIFS Fractal Foundry",
      description: "Self-similar islands gate three sharply different oscillators into a true product network, creating explosive nested spectra before harmonic excitation and a maximum-decay room.",
      family: "iterated-fold-product",
      character: "recursive glass machinery",
      category: "geometry",
      route: "KIFS Fold → Phase Distortion VCA × PM VCA × Metallic VCA → Three-way Product → Exciter → Reverb → Output",
    }, [
      node("geometry", "kifs-fold-sequencer", 20, 205, { rate: 5.15, length: 47, iterations: 7, zoom: 1.91, angle: -0.173, offset: 1.08, threshold: 0.54, scale: 5 }),
      node("phase", "phase-distortion", 245, 15, { frequency: 58.27, bend: 0.79, split: 0.27, curve: 1.62, cvDepth: 0.44, stereo: 5.2, level: 0.46 }),
      node("phase-vca", "vca", 475, 15, { base: 0, depth: 1, drive: 1.36 }),
      node("pm", "fm", 245, 190, { frequency: 43.65, ratio: 3.731, index: 5.4, level: 0.38, modDepth: 4.8 }),
      node("pm-vca", "vca", 475, 190, { base: 0, depth: 1, drive: 1.44 }),
      node("alloy", "metallic-hi-hat", 245, 365, { rate: 5.15, frequency: 540, decay: 0.16, alloy: 0.92, brightness: 0.86, open: 0.42, stereo: 0.86, level: 0.42 }),
      node("alloy-vca", "vca", 475, 365, { base: 0, depth: 1, drive: 1.28 }),
      node("product", "product-3", 710, 190, { amount: 0.74, drive: 3.2, level: 0.94 }),
      node("exciter", "harmonic-exciter", 930, 190, { drive: 2.1, warmth: 0.52, air: 0.32, even: 0.2, cvDepth: 0, mix: 0.66, ceiling: 0.9, level: 0.88 }),
      node("room", "reverb", 1150, 190, { size: 10.8, decay: 12, taps: 64, mix: 0.62, predelay: 0.16, width: 1, tone: 0.68, level: 0.78 }),
      output(1380, 190, 0.48),
    ], [
      edge("geometry", "pitch", "phase", "pitch"), edge("geometry", "gate", "phase-vca", "cv"), edge("phase", "out", "phase-vca", "signal"),
      edge("geometry", "pitch", "pm", "pitch"), edge("geometry", "gate", "pm-vca", "cv"), edge("pm", "out", "pm-vca", "signal"),
      edge("geometry", "pitch", "alloy", "pitch"), edge("geometry", "gate", "alloy-vca", "cv"), edge("alloy", "out", "alloy-vca", "signal"),
      edge("phase-vca", "out", "product", "a"), edge("pm-vca", "out", "product", "b"), edge("alloy-vca", "out", "product", "c"),
      edge("product", "out", "exciter", "signal"), edge("exciter", "out", "room", "signal"), edge("room", "out", "out", "signal"),
    ]),

    scene(227, "interference-quasicrystal-cathedral", {
      name: "Interference Quasicrystal Cathedral",
      description: "Eleven rotated plane waves carve a long quarter-tone cycle across three geometric timbres, which are amplitude-gated, reduced to bright steps, robotized, and projected into a nearly twelve-second room.",
      family: "interference-spectral-ensemble",
      character: "quasi-periodic monumental lattice",
      category: "geometry",
      route: "Interference Lattice → Terrain VCA + Wavetable VCA + Acid VCA → Three-way Sum → Quantizer → FFT Robotizer → Reverb → Output",
    }, [
      node("geometry", "interference-lattice-sequencer", 20, 205, { rate: 8.4, length: 127, axes: 11, frequency: 7.3, rotation: -0.143, threshold: 0.66, scale: 6, octaves: 4 }),
      node("terrain", "wave-terrain", 245, 15, { frequency: 38.89, radius: 0.86, centerX: -0.24, centerY: 0.31, terrain: 4, warp: 1.72, width: 0.86, level: 0.4 }),
      node("terrain-vca", "vca", 475, 15, { base: 0, depth: 1, drive: 1.24 }),
      node("table", "vector-wavetable", 245, 190, { frequency: 51.91, scan: 0.72, harmonics: 22, tilt: 0.86, stereo: 0.72, cvDepth: 0.62, level: 0.4 }),
      node("table-vca", "vca", 475, 190, { base: 0, depth: 1, drive: 1.3 }),
      node("acid", "spectral-acid", 245, 365, { frequency: 69.3, partials: 28, cutoff: 0.46, resonance: 0.92, tilt: 0.7, drive: 3.4, level: 0.36, cvDepth: 0.78 }),
      node("acid-vca", "vca", 475, 365, { base: 0, depth: 1, drive: 1.38 }),
      node("lattice", "sum-3", 710, 190, { a: 0.82, b: 0.96, c: 0.88, level: 0.78 }),
      node("steps", "quantize", 915, 190, { bits: 6, mix: 0.42, level: 0.84 }),
      node("robot", "fft-robotizer", 1115, 190, { window: 96, bins: 16, phaseSteps: 7, mix: 0.72, shift: 1.36, tilt: 0.86, width: 0.74, level: 0.84 }),
      node("room", "reverb", 1335, 190, { size: 11.6, decay: 11.2, taps: 64, mix: 0.6, predelay: 0.21, width: 1, tone: 0.74, level: 0.76 }),
      output(1565, 190, 0.46),
    ], [
      edge("geometry", "pitch", "terrain", "pitch"), edge("geometry", "gate", "terrain-vca", "cv"), edge("terrain", "out", "terrain-vca", "signal"),
      edge("geometry", "pitch", "table", "pitch"), edge("geometry", "gate", "table-vca", "cv"), edge("table", "out", "table-vca", "signal"),
      edge("geometry", "pitch", "acid", "pitch"), edge("geometry", "gate", "acid-vca", "cv"), edge("acid", "out", "acid-vca", "signal"),
      edge("terrain-vca", "out", "lattice", "a"), edge("table-vca", "out", "lattice", "b"), edge("acid-vca", "out", "lattice", "c"),
      edge("lattice", "out", "steps", "signal"), edge("steps", "out", "robot", "signal"), edge("robot", "out", "room", "signal"), edge("room", "out", "out", "signal"),
    ]),

    scene(228, "folded-coordinate-weather", {
      name: "Folded Coordinate Weather",
      description: "A continuous X/Y orbit is tiled, mirrored, and folded into polar space before two distance shapes and an interference field turn the same trajectory into related gates and phase-control streams.",
      family: "composable-geometry-field",
      character: "connected contour and moiré voices",
      category: "geometry",
      route: "Phase Plane → Tile + Mirror → Polar Fold → 2× SDF + Interference → SDF Logic → 2 gated voices → Reverb → Output",
    }, [
      node("plane", "phase-plane", 20, 205, { rate: 0.16, ratio: 1.618, phase: 0.25, xRadius: 1, yRadius: 0.86, xCenter: 0, yCenter: 0, phaseDepth: 0 }),
      node("tiles", "tile-mirror-domain", 225, 205, { repeatX: 2, repeatY: 2, rotation: 0.077, scale: 0.82, offsetX: 0.13, offsetY: -0.09, mirrorX: 1, mirrorY: 1 }),
      node("polar", "polar-fold-domain", 430, 205, { sectors: 3, rotation: 0.041, twist: 0.32, radiusGain: 1, radiusOffset: -0.18, angleGain: 1, mirror: 1, radiusFold: 0.18 }),
      node("circle", "sdf-pattern-field", 640, 35, { shape: 4, size: 0.39, repeatX: 1, repeatY: 1, rotation: 0.04, band: 0.038, softness: 0.024, invert: 0 }),
      node("box", "sdf-pattern-field", 640, 260, { shape: 1, size: 0.31, repeatX: 1, repeatY: 1, rotation: 0.125, band: 0.025, softness: 0.018, invert: 0 }),
      node("logic", "sdf-logic", 855, 115, { operation: 0, smoothing: 0.16, threshold: 0, width: 0.1, gainA: 1, gainB: 0.92, bias: 0.015, morph: 0.22 }),
      node("moire", "interference-field", 640, 425, { axes: 7, frequency: 1.2, rotation: -0.06, phase: 0.08, threshold: 0.64, softness: 0.18, gain: 2.4, bias: 0 }),
      node("contour-tone", "oscillator", 1080, 35, { frequency: 55, waveform: 2, shape: 0.42, level: 0.44, pmDepth: 4.2 }),
      node("contour-vca", "vca", 1285, 35, { base: 0, depth: 1, drive: 1.22 }),
      node("moire-tone", "fm", 1080, 270, { frequency: 41.2, ratio: 2.73, index: 3.7, level: 0.38, modDepth: 2.8 }),
      node("moire-vca", "vca", 1285, 270, { base: 0, depth: 1, drive: 1.3 }),
      node("blend", "mix", 1490, 145, { balance: -0.04, level: 0.82 }),
      node("room", "reverb", 1695, 145, { size: 8.8, decay: 9.4, taps: 64, mix: 0.56, predelay: 0.12, width: 1, tone: 0.57, level: 0.78 }),
      output(1905, 145, 0.5),
    ], [
      edge("plane", "x", "tiles", "x"), edge("plane", "y", "tiles", "y"),
      edge("tiles", "x", "polar", "x"), edge("tiles", "y", "polar", "y"),
      edge("polar", "x", "circle", "x"), edge("polar", "y", "circle", "y"),
      edge("tiles", "x", "box", "x"), edge("tiles", "y", "box", "y"),
      edge("circle", "field", "logic", "a"), edge("box", "field", "logic", "b"),
      edge("polar", "x", "moire", "x"), edge("polar", "y", "moire", "y"),
      edge("logic", "field", "contour-tone", "pm"), edge("logic", "gate", "contour-vca", "cv"), edge("contour-tone", "out", "contour-vca", "signal"),
      edge("moire", "field", "moire-tone", "index"), edge("moire", "gate", "moire-vca", "cv"), edge("moire-tone", "out", "moire-vca", "signal"),
      edge("contour-vca", "out", "blend", "a"), edge("moire-vca", "out", "blend", "b"),
      edge("blend", "out", "room", "signal"), edge("room", "out", "out", "signal"),
    ]),
  ];
