...
async function copyPresetText() {
  ...
  announce(copied ? "Complete Monstroid preset copied" : "Select the preset text and copy it manually");
}
...
async function createAudioGraph() {
  ...
  await context.audioWorklet.addModule(new URL("./monstroid-processor.js", import.meta.url));
  ...
  if (!graph) {
    ...
    if (!result.ready || !graph) {
      setAudioPresentation(
        false,
        result.error?.message || "Unable to start Monstroid audio.",
      );
      return false;
    }
  }
...
async function toggleAudio() {
  ...
  if (audioContext?.state === "running") {
    ...
    announce("Monstroid audio off");
    return;
  }
  if (await ensureAudio()) {
    announce("Monstroid audio on");
  }
}
