...
export const JULIE_SAW_RHYTHMS = freeze([
  ...
  freeze({ id: "bow-shuffle", label: "Long-short bow shuffle", description: "A long down-bow and clipped up-bow borrow the breathing lilt of the harmonica shuffle.", steps: freeze([step("bow", .92, .82, 1), rest(), step("bow", .5, .3, -1), step("bow", .76, .64, 1), rest(), step("bow", .46, .28, -1)]) }),
  freeze({ id: "train-rebows", label: "Train re-bows", description: "Hard alternating catches and short releases turn locomotive breath rhythm into rosined steel strokes.", steps: freeze([step("bow", 1, .36, 1), rest(), step("bow", .78, .28, -1), rest(), step("bow", .92, .34, 1), step("bow", .54, .2, -1), step("bow", .72, .26, 1), rest()]) }),
  freeze({ id: "tresillo-bow", label: "Tresillo 3–3–2", description: "Three uneven bow accents span eight pulses, with the final up-bow turning the phrase around.", steps: freeze([step("bow", .94, .68, 1), rest(), rest(), step("bow", .7, .56, -1), rest(), rest(), step("bow", .84, .48, 1), rest()]) }),
  freeze({ id: "triplet-answer", label: "Triplet call / answer", description: "Three descending bow calls are answered by three lighter reverse strokes.", steps: freeze([step("bow", .96, .38, 1, .1), rest(), step("bow", .78, .34, 1, .04), rest(), step("bow", .62, .3, 1, -.04), rest(), step("bow", .84, .38, -1, -.08), rest(), step("bow", .66, .34, -1, 0), rest(), step("bow", .5, .3, -1, .08), rest()]) }),
  freeze({ id: "slow-drag-bow", label: "Slow drag", description: "One heavy stroke drags through the plate, then two quiet answers leave the resonance exposed.", steps: freeze([step("bow", 1, .96, 1, -.08), step("bow", .82, .82, 1, -.04), step("bow", .56, .62, 1), rest(), step("bow", .58, .46, -1, .05), step("bow", .38, .34, -1, .09), rest(), rest()]) }),
  freeze({ id: "walking-bow", label: "Walking bow", description: "An unbroken eight-step walk alternates firm and ghosted strokes while gradually changing the arch.", steps: freeze([step("bow", .94, .52, 1, -.1), step("bow", .56, .42, -1, -.06), step("bow", .72, .48, 1, -.02), step("bow", .42, .36, -1, .02), step("bow", .88, .52, 1, .06), step("bow", .5, .4, -1, .1), step("bow", .76, .48, 1, .04), step("bow", .46, .36, -1, -.04)]) }),
  freeze({ id: "prism-waltz", label: "Prism waltz", description: "A broad first stroke and two graceful replies make a clear seated three-beat sway.", steps: freeze([step("bow", .94, .86, 1, -.06), step("bow", .54, .5, -1), step("bow", .48, .46, 1), step("bow", .82, .78, -1, .06), step("bow", .5, .48, 1), rest()]) }),
  freeze({ id: "gospel-bow-response", label: "Broad call / response", description: "A sustained bowed call opens into a layered response and a soft returning pickup.", steps: freeze([step("bow", 1, .92, 1, -.08), step("bow", .76, .7, 1, -.03), rest(), step("bow", .48, .38, -1), step("bow", .74, .58, -1, .06), step("bow", .52, .42, 1), rest(), step("bow", .34, .26, -1), step("bow", .86, .74, 1, .02), rest(), step("bow", .6, .46, -1), step("bow", .42, .3, 1)]) }),
  freeze({ id: "foxfire-chase", label: "Foxfire chase", description: "Fast darting reversals, contact feints, and one clean gap make a bright kinetic chase.", subdivision: 2, steps: freeze([step("bow", 1, .42, 1, 0, -.08), step("bow", .7, .32, -1, .04, .08), step("bow", .82, .36, 1, -.04, -.04), step("bow", .6, .28, -1, .08, .1), step("bow", .92, .4, 1, -.08, -.06), step("bow", .72, .32, -1, .04, .04), rest(), step("bow", .52, .26, 1, .1), step("bow", .7, .32, -1, 0, -.08), step("bow", .84, .36, 1, -.06, .06), step("bow", .56, .28, -1, .06), step("bow", .64, .3, 1, 0, -.04)]) }),
  freeze({ id: "smoky-rebows", label: "Smoky re-bows", description: "Half-voiced pickups and rests leave space for vibrato, body bloom, and the blade's rough edge.", steps: freeze([step("bow", .92, .72, 1, 0, .05), step("bow", .42, .34, 1, 0, -.1), rest(), step("bow", .64, .48, -1, .04), step("bow", .78, .62, 1, -.05), step("bow", .34, .28, -1, 0, .12), rest(), step("bow", .5, .4, 1)]) }),
...
export const JULIE_SAW_PRESETS = freeze([
  ...
  ["rosewood-shuffle", "Rosewood Shuffle", "concert-tenor", "continuous-bow", "bow-shuffle", { tempoBpm: 102, bend: .38, body: .58, bowBite: .64, edgeRasp: .24, stereoWidth: .46 }],
  ["prism-train", "Prism Train", "old-carpenter", "detache", "train-rebows", { tempoBpm: 138, bowPressure: .72, bowSpeed: .78, bowBite: .9, edgeRasp: .62, brightness: .7 }],
  ["tresillo-gleam", "Tresillo Gleam", "limber-soprano", "hand-vibrato", "tresillo-bow", { tempoBpm: 116, bend: .68, tipCurl: .78, brightness: .72, stereoWidth: .56 }],
  ["silver-answer", "Silver Call + Answer", "thick-stage", "clean-ring", "triplet-answer", { tempoBpm: 94, bladeDamping: .08, localization: .94, releaseSeconds: 1.18, body: .52 }],
  ["midnight-drag", "Midnight Drag", "long-baritone", "continuous-bow", "slow-drag-bow", { tempoBpm: 58, bend: .22, tipCurl: .42, brightness: .16, bowSpeed: .28, body: .72, releaseSeconds: 1.6 }],
  ["walking-chrome", "Walking Chrome", "concert-tenor", "detache", "walking-bow", { tempoBpm: 124, bend: .52, bowPressure: .54, bowSpeed: .66, brightness: .58, stereoWidth: .68 }],
  ["prism-waltz", "Prism Waltz", "concert-tenor", "knee-vibrato", "prism-waltz", { tempoBpm: 72, vibratoDepthCents: 38, vibratoRateHz: 4.6, body: .6, releaseSeconds: .92 }],
  ["gospel-blade", "Gospel Blade", "thick-stage", "clean-ring", "gospel-bow-response", { tempoBpm: 68, bend: .44, localization: .97, body: .74, releaseSeconds: 1.8, brightness: .36 }],
  ["foxfire", "Foxfire Chase", "limber-soprano", "tremolo", "foxfire-chase", { tempoBpm: 148, bend: .74, bowSpeed: .94, bowBite: .82, edgeRasp: .4, stereoWidth: .8 }],
  ["smoky-neon", "Smoky Neon", "old-carpenter", "sweep", "smoky-rebows", { tempoBpm: 88, bend: .34, bowContact: .31, localization: .54, brightness: .48, edgeRasp: .68, body: .56 }],
  ["velvet-triplets", "Velvet Triplets", "long-baritone", "knee-vibrato", "triplet-answer", { tempoBpm: 82, bowPressure: .38, bowSpeed: .36, bowBite: .4, edgeRasp: .08, brightness: .24, releaseSeconds: 1.24 }],
  ["broken-carousel", "Broken Carousel", "old-carpenter", "wowa", "prism-waltz", { tempoBpm: 104, bend: .46, vibratoDepthCents: 74, vibratoRateHz: 1.7, glideSeconds: .36, edgeRasp: .58 }],
  ["low-copper", "Low Copper", "long-baritone", "double-stop", "tresillo-bow", { tempoBpm: 76, bend: .12, tipCurl: .28, brightness: .4, body: .78, stereoWidth: .74, level: .4 }],
  ["ice-harmonic", "Ice Harmonic", "limber-soprano", "harmonic", "gospel-bow-response", { tempoBpm: 92, bend: .84, bowContact: .76, bowPressure: .3, brightness: .94, localization: .82, body: .12 }],
  ["rough-rosin", "Rough Rosin", "old-carpenter", "continuous-bow", "train-rebows", { tempoBpm: 112, bowPressure: .82, bowSpeed: .56, rosin: .94, bowBite: .96, edgeRasp: .86, brightness: .76, level: .34 }],
  ["rainbow-tremolo", "Rainbow Tremolo", "thick-stage", "tremolo", "foxfire-chase", { tempoBpm: 132, bend: .58, tipCurl: .82, bowPressure: .4, bowSpeed: .88, brightness: .66, stereoWidth: .9, body: .48 }],
...
export function flexHandlePosition(state, width = 1, height = 1) {
  ...
  const curl = clamp(state?.tipCurl, ...JULIE_SAW_LIMITS.tipCurl);
  return {
    x: Math.max(1, Number(width) || 1) * (0.39 + arch * 0.22),
