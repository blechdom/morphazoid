...
function curveWorld(_layer, _edge, from, to) {
  return { a: from, b: to };
}
...
function curveTangent(curve) {
  return {
    x: curve.b.x - curve.a.x,
    y: curve.b.y - curve.a.y,
  };
}
...
function drawCellWash() {
  ...
  state.maze.cells.forEach((cell) => {
    ...
    const polygon = cell.polygon.map(screenPoint);
    context.beginPath();
...
function drawMarker(node, color, diamond = false) {
  const current = screenPoint(state.maze.cells[node]?.center ?? { x: 0, y: 0 });
  context.save();
...
function nearestCell(clientX, clientY) {
  ...
  state.maze.cells.forEach((cell) => {
    const center = screenPoint(cell.center);
...
function svgCurvePath(curve) {
  return `M ${svgNumber(curve.a.x)} ${svgNumber(-curve.a.y)} L ${svgNumber(curve.b.x)} ${svgNumber(-curve.b.y)}`;
}
...
function exportSvg() {
  ...
  const svg = [
    ...
    state.showSolution
      ? `<path id="solution-route" data-layer="solution-route" d="${solutionPaths}" fill="none" stroke="${COLORS.solution}" stroke-width="0.007" stroke-dasharray="0.014 0.018" stroke-linecap="square"/>`
      : "",
    "</svg>",
  ].join("\n");
