...
async function loadWasmBinary(relativeUrl, { required = true } = {}) {
  try {
    const response = await fetch(new URL(relativeUrl, new URL("../../../simd-resonator-app.js", import.meta.url).href));
...
function startWorkerPrep(scalarBytes, simdBytes) {
  ...
  const worker = new Worker(new URL("../../simd-audio-worker.js", import.meta.url), { type: "module", name: "morphazoid-simd-prep" });
  state.worker = worker;
...
async function startAudio() {
  ...
  try {
    ...
    const workletPromise = context.audioWorklet.addModule(new URL("../../simd-resonator-processor.js", import.meta.url));
