...
const PROCESSOR_NAME = "morphazoid-ourorourobouroboros";
const TAU = Math.PI * 2;
const LAYER_COUNT = 21;
const LAYER_CENTER = Math.floor(LAYER_COUNT / 2);
const MAX_NOTE_LIFT = 7;
const SOURCE_LAYER_COUNT = LAYER_COUNT + MAX_NOTE_LIFT;
const DEFAULT_SAMPLE_RATE = 48_000;
const MODE_COUNT = 4;
const NOTE_FULL_BAND_MIN = 20;
const FUSION_EMPHASIS_DEPTH = 0.75;
const FUSION_SPOTLIGHT_DEPTH = 0.75;
const CONTINUUM_PITCH_ONSET_WIDTH = 2;
const CONTINUUM_PITCH_LIFT = 0.35;
...
const FAST_SINE_SIZE = 32_768;
const FAST_SINE_MASK = FAST_SINE_SIZE - 1;
const FAST_SINE = new Float32Array(FAST_SINE_SIZE);
for (let index = 0; index < FAST_SINE_SIZE; index += 1) {
  FAST_SINE[index] = Math.sin(TAU * index / FAST_SINE_SIZE);
}
...
export const OUROBOROUSEL_PRESETS = Object.freeze([
  Object.freeze({
    ...
    description: "layered rhythm-to-pitch spectrum · endless ascent",
  ...
  Object.freeze({
    id: "infinididdle",
    label: "Infinididdle",
    description: "tight layered spectrum · quick bright climb",
    ...OUROBOROUSEL_DEFAULTS,
    materialMode: "combo",
    glissRate: 0.34,
    centerRate: 7,
  ...
  Object.freeze({
    ...
    description: "early fusion · bright rhythm-to-pitch coil",
    ...OUROBOROUSEL_DEFAULTS,
    materialMode: "notes",
    glissRate: 0.2,
    centerRate: 5.5,
...
/** Table sine for nonnegative phase cycles inside the sample render loop. */
function fastSineCycle(cycles) {
  return FAST_SINE[Math.floor(cycles * FAST_SINE_SIZE) & FAST_SINE_MASK];
}
...
export function sanitizeOuroborouselParams(params = {}) {
  ...
  return Object.freeze({
    ...
    nestedSilence: clamp(
      params.nestedSilence,
      0,
      1,
      OUROBOROUSEL_DEFAULTS.nestedSilence,
    ),
    audioMix: clamp(
      params.audioMix,
      0,
      1,
      OUROBOROUSEL_DEFAULTS.audioMix,
    ),
...
export function ouroborouselWindow(
  ...
  const safeWidth = clamp(width, 3, 20, OUROBOROUSEL_DEFAULTS.bankWidth);
...
/**
 * Direct-rate pitch onset is deliberately independent of the adjustable
 * rhythm tail. A two-octave onset guarantees a genuinely mixed lane at every
 * phase of the octave-spaced lattice instead of alternating whole-bank states.
 */
export function ouroborouselPitchBlend(
  hitRate,
  fusionPoint = OUROBOROUSEL_DEFAULTS.fusionPoint,
) {
  return ouroborouselFusionBlend(
    hitRate,
    fusionPoint,
    CONTINUUM_PITCH_ONSET_WIDTH,
  );
}

/** Backward-compatible name for the shared direct-rate pitch onset. */
export function ouroborouselDrumFusionToneBlend(
  hitRate,
  fusionPoint = OUROBOROUSEL_DEFAULTS.fusionPoint,
) {
  return ouroborouselPitchBlend(hitRate, fusionPoint);
}

/**
 * One-sided rhythmic release. Pitch can form around fusionPoint while the
 * original articulation continues as far as point * 2 ** fusionWidth.
 */
export function ouroborouselRhythmShare(
  hitRate,
  fusionPoint = OUROBOROUSEL_DEFAULTS.fusionPoint,
  fusionWidth = OUROBOROUSEL_DEFAULTS.fusionWidth,
) {
  ...
  if (rate <= point) return 1;
  const normalized = Math.log2(rate / point) / width;
  if (normalized >= 1) return 0;
  return 0.5 + 0.5 * Math.cos(Math.PI * normalized);
}

/** Smooth pre-normalization lift that keeps the high direct-rate rail audible. */
export function ouroborouselPitchLift(
  pitchBlend,
) {
  const blend = clamp(pitchBlend, 0, 1, 0);
  const smooth = blend * blend * (3 - 2 * blend);
  return 1 + CONTINUUM_PITCH_LIFT * smooth;
}
...
export function ouroborouselDrumFusionGate(
  ...
  return ouroborouselContinuumGate(
    pulsePhase,
    1 - tone,
...
/** Peak-bounded pulse modulation retained on an already audible pitch. */
export function ouroborouselContinuumGate(
  pulsePhase,
  rhythmShare,
  duty = DRUM_FUSION_PULSE_DUTY,
) {
  const rhythm = clamp(rhythmShare, 0, 1, 1);
  const pulse = ouroborouselChunkEnvelope(
    pulsePhase,
    duty,
  );
  return 1 - rhythm * (1 - pulse);
}

/**
 * Let a slower, octave-related lane cut smooth holes into one high carrier.
 * A zero mix leaves the carrier untouched; a full mix becomes the lower
 * lane's Hann pulse. This is a subtractive illusion, never an added voice.
 */
export function ouroborouselNestedSilenceGate(
  pulsePhase,
  nestedMix = OUROBOROUSEL_DEFAULTS.nestedSilence,
  duty = OUROBOROUSEL_DEFAULTS.chunkDuty,
) {
  return ouroborouselContinuumGate(pulsePhase, nestedMix, duty);
}

/** Equal-power component gains, calibrated so the center preserves the mix. */
export function ouroborouselAudioMix(
  audioMix = OUROBOROUSEL_DEFAULTS.audioMix,
) {
  const mix = clamp(
    audioMix,
    0,
  ...
  return Object.freeze({
    rhythmGain: Math.cos(mix * Math.PI * 0.5) * Math.SQRT2,
    sustainGain: Math.sin(mix * Math.PI * 0.5) * Math.SQRT2,
  });
}
...
/**
 * Describe the unified rhythm/pitch bank. Slow articulation may use an
 * octave-lifted bite, but every high rail sounds its own repetition rate as
 * pitch. Both remain derivable from pulse phase and coherent at every seam.
 */
export function calculateOuroborouselLayers({
  ...
  const { rhythmGain, sustainGain } = ouroborouselAudioMix(
    safe.audioMix,
  );
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const pitchBlend = ouroborouselPitchBlend(
      hitRate,
      safe.fusionPoint,
    );
    const pitchPulseShare = 1 - pitchBlend;
    const rhythmShare = ouroborouselRhythmShare(
      hitRate,
      safe.fusionPoint,
      safe.fusionWidth,
    );
    const fusionSpotlight = ouroborouselFusionSpotlight(
      pitchBlend,
      safe.fusionEmphasis,
    );
    const pitchLift = ouroborouselPitchLift(pitchBlend);
    const pitchSafety = ouroborouselFrequencySafety(hitRate, sampleRate);
    const interruptionRate = hitRate / cyclesPerChunk;
    const interruptionOffset = octaveOffset - safe.noteLift;
    const interruptionWindow = ouroborouselWindow(
      interruptionOffset,
      safe.bankWidth,
    );
    const interruptionPhase = wrapUnit(
      OUROBOROUSEL_PHASE_SEED * 2 ** interruptionOffset,
    );
    const nestedDryWeight = window
      * pitchSafety
      * pitchBlend
      * pitchLift
      * fusionSpotlight;
    const nestedWetWeight = interruptionWindow
      * pitchSafety
      * rhythmShare
      * fusionSpotlight;
    const nestedPairWeight = Math.max(nestedDryWeight, nestedWetWeight);
    const nestedWeightTotal = nestedDryWeight + nestedWetWeight;
    const nestedMix = nestedWeightTotal > 1e-12
      ? safe.nestedSilence * nestedWetWeight / nestedWeightTotal
      : 0;
    const nestedGate = ouroborouselNestedSilenceGate(
      interruptionPhase,
      nestedMix,
      safe.chunkDuty,
    );
    ...
    const drumToneGain = pitchBlend;
    const drumHitGain = rhythmShare;
    const drumFusionFocus = ouroborouselDrumFusionFocus(
      hitRate,
      safe.fusionPoint,
    );
    const drumFusionWindow = window * fusionSpotlight;
    const drumFusionHarmonic = ouroborouselDrumFusionHarmonic(
      safe.centerRate,
    );
    const drumFusionHz = hitRate;
    const drumFusionSafety = pitchSafety;
    const drumHitWeight = window
      * drumSafety
      * drumHitGain
      * fusionSpotlight;
    const drumToneWeight = nestedPairWeight;
    ...
    const drumFusionGate = ouroborouselContinuumGate(
      pulsePhase,
      pitchPulseShare,
      DRUM_FUSION_PULSE_DUTY,
    );
  ...
  // A very low center rate, narrow bank, and large lift can place every
  // direct-rate rail below the pitch path. In Notes/Layered, keep that valid
  // setting audible with the original octave-lifted note chunks. The normal
  // nested-tone bank remains untouched whenever even one tone lane exists.
  if (noteActiveLayers === 0 && safe.materialMode !== "drums") {
    for (const layer of layers) {
      const fallbackWeight = layer.window * layer.safety;
      if (fallbackWeight <= 1e-9) continue;
      layer.noteRhythmWeight = fallbackWeight;
      layer.weight = fallbackWeight;
      layer.noteActive = true;
      noteActiveLayers += 1;
      noteTotalHitRate += layer.hitRate;
      weightPower += fallbackWeight * fallbackWeight;
      if (!layer.active) {
        layer.active = true;
        activeLayers += 1;
  ...
  const frozenLayers = layers.map((layer) => {
    const recursiveSourceIndex = layer.index + safe.noteLift;
    const recursiveSourceLayer = layers[recursiveSourceIndex] ?? null;
    const nestedRhythmIndex = layer.index - safe.noteLift;
    const nestedRhythmLayer = layers[nestedRhythmIndex] ?? null;
    const noteRhythmBankGain = layer.noteRhythmWeight * normalization;
    const noteToneBankGain = layer.noteToneWeight * normalization;
    const noteGain = Math.hypot(noteRhythmBankGain, noteToneBankGain);
    ...
    return Object.freeze({
      ...layer,
      recursiveSourceIndex: recursiveSourceLayer
        ? recursiveSourceIndex
        : null,
      recursiveSourceHz: recursiveSourceLayer?.hitRate ?? layer.sourceHz,
      recursiveSourcePhase: recursiveSourceLayer?.pulsePhase
        ?? layer.carrierPhase,
      recursiveSourceActive: Boolean(
        recursiveSourceLayer?.noteToneWeight > 1e-9,
      ),
      nestedRhythmIndex: nestedRhythmLayer ? nestedRhythmIndex : null,
      nestedRhythmHz: nestedRhythmLayer?.hitRate ?? layer.interruptionRate,
      nestedRhythmPhase: nestedRhythmLayer?.pulsePhase
        ?? layer.interruptionPhase,
      nestedRhythmActive: layer.nestedWetWeight > 1e-9,
      noteRhythmBankGain,
      noteToneBankGain,
      noteGain,
      drumHitBankGain,
...
function createProcessorClass(AudioWorkletBase) {
  return class MorphazoidOuroborouselProcessor extends AudioWorkletBase {
    constructor(options = {}) {
      ...
      this.renderPulsePhases = new Float64Array(SOURCE_LAYER_COUNT);
      this.renderPulseWraps = new Uint8Array(LAYER_COUNT);
      this.renderNoteLifts = new Uint8Array(LAYER_COUNT);
      this.renderHitRates = new Float64Array(SOURCE_LAYER_COUNT);
      this.renderOctaveOffsets = new Float64Array(LAYER_COUNT);
      this.renderWindows = new Float64Array(LAYER_COUNT);
      this.renderNoteSafeties = new Float64Array(LAYER_COUNT);
      this.renderPitchBlends = new Float64Array(LAYER_COUNT);
      this.renderRhythmShares = new Float64Array(LAYER_COUNT);
      this.renderNestedMixes = new Float64Array(LAYER_COUNT);
      this.renderNestedPhases = new Float64Array(LAYER_COUNT);
      this.renderNoteRhythmWeights = new Float64Array(LAYER_COUNT);
      this.renderNoteToneWeights = new Float64Array(LAYER_COUNT);
      this.renderRawDrumWeights = new Float64Array(LAYER_COUNT);
      this.renderDrumHitWeights = new Float64Array(LAYER_COUNT);
      this.renderDrumToneWeights = new Float64Array(LAYER_COUNT);
      this.sourceRendered = new Uint8Array(SOURCE_LAYER_COUNT);
      this.notePitchCores = new Float64Array(SOURCE_LAYER_COUNT);
      this.notePitchSamples = new Float64Array(SOURCE_LAYER_COUNT);
      this.drumPitchSamples = new Float64Array(SOURCE_LAYER_COUNT);
    ...
    snapCurrentToTarget() {
      ...
      this.current.nestedSilence = this.target.nestedSilence;
      this.current.audioMix = this.target.audioMix;
    ...
    renderPitchSource(
      ...
      if (this.sourceRendered[index] !== 0) return;
      this.sourceRendered[index] = 1;
      const pulsePhase = this.renderPulsePhases[index];
      const sourceHitRate = this.renderHitRates[index];
      const pitchBlend = ouroborouselPitchBlend(
        sourceHitRate,
        this.current.fusionPoint,
      );
      const pitchPulseShare = 1 - pitchBlend;
      const fundamental = fastSineCycle(pulsePhase);
      ...
      if (renderNoteColor) {
        ...
        const noteCore = (
          fundamental
          + fastSineCycle(pulsePhase * 2)
            * brightness * 0.28 * secondSafety
          + fastSineCycle(pulsePhase * 3)
            * brightness * brightness * 0.12 * thirdSafety
        ) / carrierDenominator;
        this.notePitchCores[index] = noteCore;
        this.notePitchSamples[index] = noteCore
        ...
        this.notePitchCores[index] = 0;
        this.notePitchSamples[index] = 0;
      }

      if (renderDrumColor) {
        const drumSourceIndex = Math.min(index, LAYER_COUNT - 1);
        const drumModeBase = drumSourceIndex * MODE_COUNT;
        ...
        for (let mode = 0; mode < MODE_COUNT; mode += 1) {
          ...
          const modeGain = this.drumModalGain[stateIndex]
            * ouroborouselFrequencySafety(
              sourceHitRate * harmonic,
              workletSampleRate,
            );
          if (modeGain <= 1e-12) continue;
          drumModalSample += fastSineCycle(
            pulsePhase * harmonic,
          ) * modeGain;
          drumModalGain += modeGain;
        }
        if (drumModalGain > 1e-12) drumModalSample /= drumModalGain;
        const drumCore = fundamental * 0.72 + drumModalSample * 0.28;
        this.drumPitchSamples[index] = drumCore
          * ouroborouselContinuumGate(
            pulsePhase,
            pitchPulseShare,
            DRUM_FUSION_PULSE_DUTY,
    ...
    process(_inputs, outputs) {
      ...
      for (let sampleIndex = 0; sampleIndex < left.length; sampleIndex += 1) {
        ...
        this.current.nestedSilence += (
          this.target.nestedSilence - this.current.nestedSilence
        ) * parameterSlew;
        this.current.audioMix += (
          this.target.audioMix - this.current.audioMix
        ) * parameterSlew;
        this.current.spread += (
        ...
        const firstOffset = -LAYER_CENTER + this.position;
        const firstHitRate = this.current.centerRate * 2 ** firstOffset;
        const advancePhases = this.activeTarget > 0.5 || manualEnvelope > 1e-7;
        const brightness = this.current.brightness;
        const carrierDenominator = 1
          + brightness * 0.28
          + brightness * brightness * 0.12;
        const automaticAudible = this.transportTarget > 0.5
          || this.transportGain > 1e-7;
        const renderNoteColor = this.currentMaterialMix < 0.999999
        ...
        let sourceHitRate = firstHitRate;

        // Capture every real and ghost source phase before advancing the
        // transport. Pitch waveforms are rendered lazily when a lane consumes
        // them below, so each future source is still computed at most once.
        for (let index = 0; index < SOURCE_LAYER_COUNT; index += 1) {
          let pulsePhase;
          if (index < LAYER_COUNT) {
            pulsePhase = this.pulsePhases[index];
            const noteLift = this.noteLifts[index];
            this.renderPulsePhases[index] = pulsePhase;
            this.renderNoteLifts[index] = noteLift;
            let pulseWraps = 0;
            if (advancePhases) {
              const nextPhase = pulsePhase + sourceHitRate / workletSampleRate;
              pulseWraps = Math.floor(nextPhase);
              this.pulsePhases[index] = nextPhase - pulseWraps;
              // Apply a changed source lift after this zero-crossing sample.
              if (pulseWraps > 0 && noteLift !== this.target.noteLift) {
                this.noteLifts[index] = this.target.noteLift;
              }
            }
            this.renderPulseWraps[index] = pulseWraps;
            const octaveOffset = firstOffset + index;
            const distance = Math.abs(octaveOffset) / halfWidth;
            const pitchBlend = ouroborouselPitchBlend(
              sourceHitRate,
            ...
            let window = 0;
            let noteSafety = 0;
            let noteRhythmWeight = 0;
            const pitchSafety = ouroborouselFrequencySafety(
              sourceHitRate,
              workletSampleRate,
            );
            let rawDrumWeight = 0;
            ...
            if (distance < 1) {
              ...
              if (window > 1e-7) {
                noteSafety = ouroborouselFrequencySafety(
                  sourceHitRate * 2 ** noteLift,
                  workletSampleRate,
                );
                const drumFundamental = DRUM_CENTER_PITCH * 2 ** octaveOffset;
                rawDrumWeight = window * ouroborouselFrequencySafety(
                  drumFundamental,
                  workletSampleRate,
                );
                drumHitWeight = rawDrumWeight * rhythmShare;
              }
            }
            const nestedRhythmIndex = index - noteLift;
            const nestedWindow = nestedRhythmIndex >= 0
              ? this.renderWindows[nestedRhythmIndex]
              : 0;
            const nestedDryWeight = window
              * pitchSafety
              * pitchBlend
              * pitchLift
              * fusionSpotlight;
            const nestedWetWeight = nestedWindow
              * pitchSafety
              * rhythmShare
              * fusionSpotlight;
            const nestedWeightTotal = nestedDryWeight + nestedWetWeight;
            ...
            const drumToneWeight = noteToneWeight;
            this.renderNestedMixes[index] = nestedWeightTotal > 1e-12
              ? this.current.nestedSilence
                * nestedWetWeight / nestedWeightTotal
              : 0;
            this.renderNestedPhases[index] = nestedRhythmIndex >= 0
              ? this.renderPulsePhases[nestedRhythmIndex]
              : 0;
            if (noteToneWeight > 1e-9) {
              noteTonePresent = true;
              if (noteSafety === 1 || pitchSafety === 1) {
                noteMaterialReady = true;
              }
            }
            this.renderOctaveOffsets[index] = octaveOffset;
            this.renderWindows[index] = window;
            this.renderNoteSafeties[index] = noteSafety;
            this.renderPitchBlends[index] = pitchBlend;
            this.renderRhythmShares[index] = rhythmShare;
            this.renderNoteRhythmWeights[index] = noteRhythmWeight;
            this.renderNoteToneWeights[index] = noteToneWeight;
            this.renderRawDrumWeights[index] = rawDrumWeight;
            this.renderDrumHitWeights[index] = drumHitWeight;
            this.renderDrumToneWeights[index] = drumToneWeight;
          } else {
            pulsePhase = wrapUnit(this.renderPulsePhases[index - 1] * 2);
            this.renderPulsePhases[index] = pulsePhase;
          }
          this.renderHitRates[index] = sourceHitRate;
          sourceHitRate *= 2;
        }

        // Preserve the nested high-rail sound exactly when it exists. If the
        // entire tone bank falls below the pitch path, restore the phase-safe
        // octave-lifted note chunks so returning from Drums cannot lock on or
        // fade into silence at valid low/narrow/high-lift settings.
        if (!noteTonePresent && renderNoteColor) {
          for (let index = 0; index < LAYER_COUNT; index += 1) {
            const fallbackWeight = this.renderWindows[index]
              * this.renderNoteSafeties[index];
            this.renderNoteRhythmWeights[index] = fallbackWeight;
            if (
              fallbackWeight > 1e-9
              && this.renderNoteSafeties[index] === 1
            ) {
              noteMaterialReady = true;
            }
          }
        ...
        let noteRhythmLeftSample = 0;
        let noteRhythmRightSample = 0;
        let noteSustainLeftSample = 0;
        let noteSustainRightSample = 0;
        let noteManualLeftSample = 0;
        let noteManualRightSample = 0;
        let drumRhythmLeftSample = 0;
        let drumRhythmRightSample = 0;
        let drumSustainLeftSample = 0;
        let drumSustainRightSample = 0;
        let drumManualLeftSample = 0;
        let drumManualRightSample = 0;
        let noteWeightPower = 0;
        let drumWeightPower = 0;

        for (let index = 0; index < LAYER_COUNT; index += 1) {
          const pulsePhase = this.renderPulsePhases[index];
          const noteLift = this.renderNoteLifts[index];
          const pulseWraps = this.renderPulseWraps[index];
          const hitRate = this.renderHitRates[index];
          const octaveOffset = this.renderOctaveOffsets[index];
          const window = this.renderWindows[index];
          const noteSafety = this.renderNoteSafeties[index];
          const pitchBlend = this.renderPitchBlends[index];
          const rhythmShare = this.renderRhythmShares[index];
          const nestedMix = this.renderNestedMixes[index];
          const nestedPhase = this.renderNestedPhases[index];
          const noteRhythmWeight = this.renderNoteRhythmWeights[index];
          const noteToneWeight = this.renderNoteToneWeights[index];
          const rawDrumWeight = this.renderRawDrumWeights[index];
          const drumHitWeight = this.renderDrumHitWeights[index];
          const manualDrumWeight = rawDrumWeight * manualDrumBypass;
          const drumToneWeight = this.renderDrumToneWeights[index];

          // Manual grabs deliberately excite the whole Shepard bank. Pointer
          // position places the visual playhead; it must not collapse the
          // sound back to one isolated octave lane.
          const manualWeight = 1;

          if (rawDrumWeight > 1e-9) {
            let strikeVelocity = transportActive
              && pulseWraps > 0
              && rhythmShare > 1e-7
              ? 1
              : 0;
          ...
          const pan = Math.max(-1, Math.min(1, normalizedOffset))
            * this.current.spread;
          const panCycle = (pan + 1) * 0.125;
          const leftPan = fastSineCycle(panCycle + 0.25);
          const rightPan = fastSineCycle(panCycle);

          const manualNoteWeight = window * noteSafety * manualWeight;
          const recursiveSourceIndex = index + noteLift;
          const noteVoiceNeeded = renderNoteColor && (
            noteRhythmWeight > 1e-9
            || noteToneWeight > 1e-9
            || (manualEnvelope > 1e-7 && manualNoteWeight > 1e-9)
          );
          const drumVoiceNeeded = renderDrumColor && (
            drumHitWeight > 1e-9
            || drumToneWeight > 1e-9
            || manualDrumWeight > 1e-9
          );
          const needsRecursiveSource = noteSafety > 1e-9
            && noteVoiceNeeded
            && (
              (automaticAudible && noteRhythmWeight > 1e-9)
              || (manualEnvelope > 1e-7 && manualNoteWeight > 1e-9)
            );
          const needsDirectSource = automaticAudible && (
            (noteVoiceNeeded && noteToneWeight > 1e-9)
            || (drumVoiceNeeded && drumToneWeight > 1e-9)
          );
          if (needsRecursiveSource) {
            this.renderPitchSource(
              recursiveSourceIndex,
              workletSampleRate,
              renderNoteColor,
              renderDrumColor,
          ...
          if (noteVoiceNeeded) {
            const chunkEnvelope = ouroborouselChunkEnvelope(
              pulsePhase,
              this.current.chunkDuty,
            );
            const nestedPulse = ouroborouselChunkEnvelope(
              nestedPhase,
              this.current.chunkDuty,
            );
            const automaticRhythmSample = this.notePitchCores[
              recursiveSourceIndex
            ] * this.transportGain * noteRhythmWeight * chunkEnvelope;
            const automaticPitchSample = this.notePitchSamples[index]
              * this.transportGain
              * noteToneWeight;
            const sustainSample = automaticPitchSample * (1 - nestedMix);
            const rhythmSample = automaticPitchSample
              * nestedMix
              * nestedPulse;
            const manualSample = this.notePitchCores[recursiveSourceIndex]
              * manualEnvelope
              * manualNoteWeight;
            noteRhythmLeftSample += (
              automaticRhythmSample + rhythmSample
            ) * leftPan;
            noteRhythmRightSample += (
              automaticRhythmSample + rhythmSample
            ) * rightPan;
            noteSustainLeftSample += sustainSample * leftPan;
            noteSustainRightSample += sustainSample * rightPan;
            noteManualLeftSample += manualSample * leftPan;
            noteManualRightSample += manualSample * rightPan;
            noteWeightPower += noteRhythmWeight * noteRhythmWeight
              + noteToneWeight * noteToneWeight;
            if (manualEnvelope > 1e-7) {
              noteWeightPower += manualNoteWeight * manualNoteWeight;
            }
          }
          if (drumVoiceNeeded) {
            const nestedPulse = ouroborouselChunkEnvelope(
            ...
            // Drum attacks and modal bodies remain fully authentic. The
            // phase-locked rail sits behind them and receives only subtractive
            // holes from its slower octave partner.
            const automaticDrumRhythm = drumVoiceSample
              * this.transportGain
              * drumHitWeight;
            const automaticDrumPitch = this.drumPitchSamples[index]
              * this.transportGain
              * drumToneWeight;
            const sustainSample = automaticDrumPitch * (1 - nestedMix);
            const rhythmSample = automaticDrumRhythm
              + automaticDrumPitch * nestedMix * nestedPulse;
            const manualSample = drumVoiceSample * manualDrumWeight;
            drumRhythmLeftSample += rhythmSample * leftPan;
            drumRhythmRightSample += rhythmSample * rightPan;
            drumSustainLeftSample += sustainSample * leftPan;
            drumSustainRightSample += sustainSample * rightPan;
            drumManualLeftSample += manualSample * leftPan;
            drumManualRightSample += manualSample * rightPan;
            drumWeightPower += drumHitWeight * drumHitWeight
              + drumToneWeight * drumToneWeight;
            if (manualDrumWeight > 1e-9) {
              drumWeightPower += manualDrumWeight * manualDrumWeight;
            }
          }
          if (pulseWraps > 0) {
            for (let mode = 0; mode < MODE_COUNT; mode += 1) {
        ...
        const drumNormalization = drumWeightPower > 1e-12
          ? 1 / Math.sqrt(drumWeightPower)
          : 0;
        const audioMixCycle = this.current.audioMix * 0.25;
        const rhythmMixGain = fastSineCycle(audioMixCycle + 0.25)
          * Math.SQRT2;
        const sustainMixGain = fastSineCycle(audioMixCycle)
          * Math.SQRT2;
        const noteMaterialGain = Math.cos(
          this.currentMaterialMix * Math.PI * 0.5,
        );
        const noteLeftSample = noteRhythmLeftSample * rhythmMixGain
          + noteSustainLeftSample * sustainMixGain
          + noteManualLeftSample;
        const noteRightSample = noteRhythmRightSample * rhythmMixGain
          + noteSustainRightSample * sustainMixGain
          + noteManualRightSample;
        const drumLeftSample = drumRhythmLeftSample * rhythmMixGain
          + drumSustainLeftSample * sustainMixGain
          + drumManualLeftSample;
        const drumRightSample = drumRhythmRightSample * rhythmMixGain
          + drumSustainRightSample * sustainMixGain
          + drumManualRightSample;
...
/** Lazy, autoplay-safe Web Audio wrapper for the Ourorourobouroboros worklet. */
export class OuroborouselAudio {
  ...
  async initialize() {
    ...
    if (!context.audioWorklet) {
      await context.close();
      throw new Error("Ourorourobouroboros requires AudioWorklet support.");
    }
    ...
    try {
      ...
      await context.audioWorklet.addModule(
        new URL("./ourorourobouroboros.js", import.meta.url),
      );
      const WorkletNode = this.runtime.AudioWorkletNode
        ?? globalThis.AudioWorkletNode;
      if (typeof WorkletNode !== "function") {
        throw new Error("Ourorourobouroboros requires AudioWorkletNode support.");
      }
