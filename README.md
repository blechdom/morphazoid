...
**Shape** is a two-dimensional instrument: one to twelve point heads trace the boundary, one to four independently orientable scan lines read contour intersections, or center-rooted radar rays rotate through every contour hit. Every playhead type can loop or ping-pong. Point and radar spacing markers carry independent forward/reverse arrows, while each line marker carries its own 90-degree orientation toggle. Scan lines stay inside the contour's projected bounds. Reader and rotation transports run independently from zero to four cycles or revolutions per second. Sine with motion-gated corner emphasis is the default voice; Form count 1 is a vertex-free circle with constant gain and no corner envelope, 2 is an open line, and 3–32 are polygons.
...
Head spacing can be edited while the instrument runs: drag the markers in the compact head-layout control, or reset them to an even distribution. Form count chooses circle, open line, or polygon; signed controls then stretch, skew, round outward, or bow inward, with a center reset beside each slider. Every transform is normalized so the whole rotating form remains inside the stage. Drag the contour itself to spin it whenever auto-rotation is off.
