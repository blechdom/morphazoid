...
test("Shapes: all 117 recalls are exact, update dimension/motion and never arm Audio", async ({ page, baseURL }) => {
...
test("Shapes: every factory scene produces bounded audio after an explicit Audio action", async ({ page, baseURL }, testInfo) => {
  ...
  await testInfo.attach("all-117-audio.json", { body: JSON.stringify(results, null, 2), contentType: "application/json" });
  expect(pageDiagnosticMessages(diagnostics)).toEqual([]);
});
