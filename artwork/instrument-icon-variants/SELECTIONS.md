# Instrument Icon Selection Queue

The order below matches the site menu. A checked item has been selected and
copied to `assets/instruments/`. Each unchecked item links to its decision
folder.

## Geometry Synths

1. [x] Shape - [catalogue-preferred.webp](shape/catalogue-preferred.webp)
2. [ ] Lattice - [open folder](lattice/)
3. [ ] Spiral - [open folder](spiral/)
4. [ ] Solid - [open folder](solid/)
5. [ ] Hyper - [open folder](hyper/)

## Geometry Drum Machines

6. [x] Shape Drum Machine - [round-1-original.webp](shape-drums/round-1-original.webp)
7. [ ] Lattice Drum Machine - [open folder](lattice-drums/)
8. [ ] Spiral Drum Machine - [open folder](spiral-drums/)
9. [ ] Solid Drum Machine - [open folder](solid-drums/)
10. [ ] Hyper Drum Machine - [open folder](hyper-drums/)
11. [ ] L-System Drum Machine - [open folder](l-system-drums/)
12. [ ] Rattle Snake Skin - [open folder](linear-drums-machine/)

## Signal & Voice

13. [ ] Lumber Loops - [open folder](lumber/)
14. [ ] L-system Delay - [open folder](micmic/)
15. [ ] Graph Delay - [open folder](graph-delay/)
16. [ ] Throatazoid - [open folder](throatazoid/)

## Barber Shop Poles

17. [ ] Shepard-Risset - [open folder](shepard-risset/)
18. [ ] Sandy Syrup Delay - [open folder](sandy-syrup-delay/)
19. [ ] Striped Sludge Delay - [open folder](striped-sludge-delay/)
20. [ ] Candy Coil Delay - [open folder](candy-coil-delay/)

## Fractals & Recursion

21. [ ] L-System - [open folder](l-system/)
22. [ ] Recursion - [open folder](recursion/)
23. [ ] Julia - [open folder](julia/)

## Chaotic Synths

24. [ ] Recursive FM - [open folder](recursive-fm/)
25. [ ] Recursive PM - [open folder](recursive-pm/)
26. [ ] Chaotic FM - [open folder](chaotic-fm/)
27. [ ] Chaotic PM - [open folder](chaotic-pm/)
28. [ ] Weierstrass - [open folder](weierstrass/)

## WebGPU Synths

29. [ ] WebGPU 303 - [open folder](webgpu-303/)

## Instruments

30. [ ] FM Drums - [open folder](fm-drums/)
31. [ ] Rattlesnake - [open folder](linear-drums/)
32. [ ] Sample Drums - [open folder](sample-drums/)

## Algorithmic Sequencers

33. [ ] Sorting - [open folder](sorting-algorithms/)
34. [ ] DJ Dijkstra - [open folder](dijkstra/)
35. [ ] Hanoi Carillon - [open folder](hanoi/)
36. [ ] Alpha-Beta Minimax - [open folder](minimax/)
37. [ ] N-Queens Backtracker - [open folder](nqueens/)
38. [ ] Euclidean Pulse - [open folder](euclid/)

## Experiments

39. [ ] Order Tones - [open folder](order-tones/)
40. [ ] Morphazoidical - [open folder](morphazoidical/)
41. [ ] Bell Square - [open folder](bell-square/)
42. [ ] Annealogue - [open folder](annealogue/)
43. [ ] Gravity Walk - [open folder](gravity-walk/)
44. [ ] Ricochet - [open folder](ricochet/)
45. [ ] Rigidity - [open folder](rigidity/)
46. [ ] Rolling Measure - [open folder](rolling-measure/)
47. [ ] Falling Forms - [open folder](falling-forms/)
48. [ ] Charge Garden - [open folder](charge-garden/)
49. [ ] Packing Pressure - [open folder](packing-pressure/)
50. [ ] Geodesic Drift - [open folder](geodesic-drift/)
51. [ ] Kinetic Hull - [open folder](kinetic-hull/)
52. [ ] RISSET-MOIRE - [open folder](moire-organ/)
53. [ ] Chladni Plate - [open folder](chladni-plate/)
54. [ ] Spring Choir - [open folder](spring-choir/)
55. [ ] Gear Ratio Drums - [open folder](gear-ratio-drums/)
56. [ ] Cellular Automata - [open folder](cellular-automata/)
57. [ ] Prime Sieve - [open folder](prime-sieve/)
58. [ ] Lissajous Orbits - [open folder](lissajous-orbits/)
59. [ ] Pendulum Wave - [open folder](pendulum-wave/)
60. [ ] Double Pendulum - [open folder](double-pendulum/)
61. [ ] Reaction-Diffusion - [open folder](reaction-diffusion/)
62. [ ] Atomic Orbitals - [open folder](atomic-orbitals/)
63. [ ] DNA Translator - [open folder](dna-translator/)
64. [ ] Neural Pulse - [open folder](neural-pulse/)
65. [ ] Fourier Epicycles - [open folder](fourier-epicycles/)
66. [ ] Gravity Lens - [open folder](gravity-lens/)

## Unlisted Artwork

67. [ ] Algorithmic Scores redirect icon - [open folder](algorithmic-scores/)
