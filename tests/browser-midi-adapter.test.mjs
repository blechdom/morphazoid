...
test("one acyclic capability registry covers every playable catalog instrument", () => {
  ...
  assert.deepEqual(
    ...
    { processor: 6, drums: 16, pitched: 34, sequence: 33 },
    "all 89 routes have exactly one intentional note behavior",
  );
  assert.equal(
...
test("every playable catalog page loads shared browser MIDI and exposes a toolbar host", async () => {
  let mastheadPages = 0;
