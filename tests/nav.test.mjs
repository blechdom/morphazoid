...
test("active tool resolution preserves GitHub Pages subpaths and nested workbench pages", () => {
  ...
  assert.equal(
    resolveActiveTool(`${SITE_ROOT}throat-singing.html`, SITE_ROOT)?.id,
    "throat-singing",
  );
  ...
  assert.equal(
    resolveActiveTool(`${SITE_ROOT}ourorourobouroboros.html`, SITE_ROOT)?.id,
    "ourorourobouroboros",
  );
  ...
  assert.equal(resolveActiveTool(`${SITE_ROOT}simd-synth.html`, SITE_ROOT)?.id, "simd-synth");
