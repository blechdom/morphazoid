...
test("Automatapoeia preserves exact live evolution while exposing history-safe interventions and continuous mono sound", async () => {
  ...
  assert.match(html, /id="automataVoiceTitle">Sound<\/h2>/);
  ...
  const voice = html.indexOf('id="caVoice"');
  const sonificationMode = html.indexOf('id="caSonificationMode"');
  const frequencyMin = html.indexOf('id="caFrequencyMin"');
  const frequencyMax = html.indexOf('id="caFrequencyMax"');
  const soundDetails = html.indexOf('id="caSoundDetails"');
  ...
  assert.ok(soundDetails < html.indexOf('id="caPolarity"'));
  assert.ok(soundDetails < html.indexOf('id="caObjectMode"'));
