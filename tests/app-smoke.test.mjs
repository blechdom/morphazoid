...
test("app.js initializes and draws one frame against browser APIs", async () => {
  ...
  assert.equal(elements.get("cornerAmplitudeMapping").hidden, false);
  assert.equal(elements.get("cornerAmplitudeSource").value, "fixed");
  assert.equal(elements.get("cornerAmplitudeSourceFieldLabel").textContent, "Corner ADSR level source");
  assert.equal(elements.get("cornerAmplitudeMappingNote").textContent, "Direct control → Corner ADSR level · 100%");
  ...
  assert.equal(attributes.get("cornerSwellToggle:aria-pressed"), "false");
  assert.equal(elements.get("cornerSwellToggleText").textContent, "Swell off");
  elements.get("cornerAmplitudeSource").value = "horizontal";
  listeners.get("cornerAmplitudeSource:change")({ currentTarget: elements.get("cornerAmplitudeSource") });
  assert.equal(elements.get("cornerAmplitudeSourceFieldLabel").textContent, "Synth level source");
  assert.equal(elements.get("cornerAmplitudeMappingNote").textContent, "Left to right → Synth level · 0–100%");
  assert.match(elements.get("cornerAmplitudeSourceHelp").textContent, /Corner ADSR is bypassed/);
  elements.get("position").value = "0.25";
  listeners.get("position:input")();
  queuedFrame(1_001);
  const rightMappedGain = Number(elements.get("markGainOut").textContent);
  elements.get("position").value = "0.75";
  listeners.get("position:input")();
  queuedFrame(1_002);
  const leftMappedGain = Number(elements.get("markGainOut").textContent);
  assert.ok(rightMappedGain > 0.17, `expected full mapped gain at stage right, received ${rightMappedGain}`);
  assert.equal(leftMappedGain, 0, "Left → right must reach zero at the stage-left edge");
  elements.get("cornerAmplitudeSource").value = "fixed";
  listeners.get("cornerAmplitudeSource:change")({ currentTarget: elements.get("cornerAmplitudeSource") });
  ...
  assert.equal(elements.get("cornerAmplitudeMapping").hidden, true);
  ...
  assert.equal(elements.get("timbreSourceFieldLabel").textContent, "FM index source");
  assert.equal(elements.get("timbreSource").value, "fixed");
  ...
  assert.equal(elements.get("timbreMapping").hidden, false);
  assert.equal(elements.get("timbreSourceFieldLabel").textContent, "PM phase-depth source");
  assert.equal(elements.get("timbreSource").value, "fixed", "PM must retain its own Direct default");
  assert.equal(elements.get("timbreMappingNote").textContent, "Direct control → Phase depth · 2.00 rad");
  elements.get("timbreSource").value = "center";
  listeners.get("timbreSource:change")({ currentTarget: elements.get("timbreSource") });
  assert.equal(elements.get("timbreMappingNote").textContent, "Center to edge → Phase depth · 0–2.00 rad");
  queuedFrame(1_143);
  assert.equal(elements.get("markSynthDriveOut").textContent, elements.get("markCenterOut").textContent);
  assert.match(elements.get("markSynthValueOut").textContent, /rad @/);
  elements.get("soundMode").value = "fm";
  listeners.get("soundMode:change")({ currentTarget: elements.get("soundMode") });
  assert.equal(elements.get("timbreSource").value, "corner", "FM must retain its Corner source");
  elements.get("soundMode").value = "pm";
  listeners.get("soundMode:change")({ currentTarget: elements.get("soundMode") });
  assert.equal(elements.get("timbreSource").value, "center", "PM must retain its Center source");
  ...
  elements.get("cornerAmplitudeSource").value = "center";
  listeners.get("cornerAmplitudeSource:change")({ currentTarget: elements.get("cornerAmplitudeSource") });
  elements.get("soundMode").value = "fm";
  listeners.get("soundMode:change")({ currentTarget: elements.get("soundMode") });
  ...
  elements.get("soundMode").value = "pm";
  listeners.get("soundMode:change")({ currentTarget: elements.get("soundMode") });
  elements.get("timbreSource").value = "horizontal";
  listeners.get("timbreSource:change")({ currentTarget: elements.get("timbreSource") });
  assert.equal(elements.get("timbreSource").value, "horizontal");
  elements.get("soundMode").value = "fm";
  listeners.get("soundMode:change")({ currentTarget: elements.get("soundMode") });
  ...
  assert.equal(elements.get("cornerAmplitudeSource").value, "center");
  assert.equal(elements.get("timbreSource").value, "phase");
